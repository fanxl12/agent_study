...
decomposition_chain = (
        ...
        | ChatOpenAI(model="kimi-k2-turbo-preview", temperature=0)
        | StrOutputParser()
...
# 4. 链式调用map()实现并行批量处理
batch_result = chain.map().invoke(sub_questions)

# 5.循环遍历所有子问题答案进行合并
qa_pairs = ""
for idx, answer in enumerate(batch_result):
...
# 6.构建最终的并行问答链：提示模板+链
end_prompt = ChatPromptTemplate.from_template("""这是你需要回答的问题：
---
{question}
---
...
end_answer = end_chain.invoke({"question": question, "qa_pairs": qa_pairs})
print(f"最终答案: {end_answer}")
client.close()
